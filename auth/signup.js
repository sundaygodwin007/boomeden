...
// declared a variable called countryData instead of using a drop down button in the HTML file i used this instead so that the user cant change the country code
const countryData = {
  Nigeria: { code: "+234", age: 18 },
  Ghana: { code: "+233", age: 18 },
  USA: { code: "+1", age: 18 },
  UK: { code: "+44", age: 18 },
  Canada: { code: "+1", age: 18 }
};

// 1. AUTO COUNTRY CODE WHEN COUNTRY CHANGES
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if(defaultCountry && countryData[defaultCountry]) {
        document.getElementById('countryCode').value = countryData[defaultCountry].code;
    }
});
...
// This function generates a one-time password, sends it through EmailJS, and stores the temporary signup data for the next verification step.
...
.catch(async (err) => {
  ...
  // REMOVED user.delete() BECAUSE IT CAUSES ERROR IF USER DOESNT EXIST
  alert("EmailJS Error: " + err.text); // this will tell us why it failed
  console.log(err);
});
}

// Attach the signup button click handler so the form validation and Firebase account creation can run when the user presses Sign Up.
document.getElementById('signupBtn').addEventListener('click', async (e) => {
    ...
    // AGE CHECK
    // declared a variable called dob and gave it a variable name called new new date(dateofbirth)
const dob = new Date(dateofbirth); // convert string to Date object

// declared a new variable called today and called the variable name new date that i declared first
      ...
      if(age < 18){
        alert("You must be 18+ to sign up on Boomeden");
        return;
      }
...
// This block tries to create the Firebase Authentication account first, then stores a temporary profile document before sending the OTP.
try {
...
if(window.location.pathname.includes('auth/verifyOTP.html')){
...
async function verifyOTP() {
    ...
    try {
        await setDoc(doc(db, "users", tempUserData.uid), {
            ...
            phone: tempUserData.fullPhone, // CHANGED FROM tempUserData.phone
            createdAt: new Date(),
            isVerified: true,
            boomCredits: 0,
            boomScore: 0
