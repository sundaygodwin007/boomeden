...
// Firebase Setup
  ...
  // This section initializes Firebase for the OTP verification page so the code can read and write the OTP flow data.
    ...
    // This handler runs when the Verify button is clicked and checks whether the entered OTP is valid.
...
await db.collection("users").doc(tempUser.uid).set({
      ...
      // this is to save the user data to the local storage so that it can be used in the dashboard and other pages
...
// ===== END NEW CODE =====
  ...
  // This variable stores the timer interval so it can be started and stopped while the resend button is on cooldown.
  let countdownTimer;
...
// This function disables the resend button for a short time and shows a countdown so the user cannot spam OTP requests.
